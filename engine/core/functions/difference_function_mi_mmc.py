...
from posetimation.loss.mse_loss import JointMSELoss, JointSequenceMSELoss
...
class DifferenceFunctionMI1(BaseFunction):
    ...
    def train(self, model, epoch, optimizer, dataloader, tb_writer_dict, **kwargs):
        ...
        for iter_step in tqdm(list(range(self.max_iter_num))):
            ...
            pred_heatmaps, hrnet_heatmaps, mi_loss_list = model(input_x.cuda(), sup_x.cuda(), iter=iter_step)
            ...
            if len(mi_loss_list) > 0:
                ...
                # loss_mi, mi_weight, alpha, beta, gama = 0, 1, 0.1, 0.1, 0.1

                mi_fm_fn, mi_fm_f, mi_fm_fa, mi_fm_ff, mi_fa_ff, mi_fm_y, mi_fa_y = mi_loss_list

                motion_mi_loss = 2 * mi_fm_fn - mi_fm_f
